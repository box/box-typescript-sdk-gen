...
export class ZipDownloadsManager {
    ...
    async postZipDownloads(requestBody: ZipDownloadRequest): Promise<any> {
        const response: FetchResponse = await fetch("".concat("https://api.box.com/2.0/zip_downloads") as string, { method: "POST", body: await serializeZipDownloadRequest(await serializeJson(requestBody)), auth: this.auth } satisfies FetchOptions) as FetchResponse;
        return await deserializeZipDownload(await deserializeJson(response.text));
    }
    ...
    async getZipDownloadsIdStatus(zipDownloadId: string): Promise<any> {
        ...
        return await deserializeZipDownloadStatus(await deserializeJson(response.text));
    }
}
