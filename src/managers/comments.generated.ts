...
export class CommentsManager {
    ...
    async updateCommentById(commentId: string, requestBody: UpdateCommentByIdRequestBodyArg, options: UpdateCommentByIdOptionsArg = {} satisfies UpdateCommentByIdOptionsArg): Promise<any> {
        const response: FetchResponse = await fetch("".concat("https://api.box.com/2.0/comments/", commentId) as string, { method: "PUT", params: { ["fields"]: options.fields }, body: JSON.stringify(requestBody), contentType: "application/json", auth: this.auth } satisfies FetchOptions) as FetchResponse;
        return deserializeCommentFull(deserializeJson(response.text));
    }
    ...
    async createComment(requestBody: CreateCommentRequestBodyArg, options: CreateCommentOptionsArg = {} satisfies CreateCommentOptionsArg): Promise<any> {
        const response: FetchResponse = await fetch("".concat("https://api.box.com/2.0/comments") as string, { method: "POST", params: { ["fields"]: options.fields }, body: JSON.stringify(requestBody), contentType: "application/json", auth: this.auth } satisfies FetchOptions) as FetchResponse;
