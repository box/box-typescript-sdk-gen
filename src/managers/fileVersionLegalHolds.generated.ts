...
export class FileVersionLegalHoldsManager {
    ...
    async getFileVersionLegalHolds(policyId: string, options: GetFileVersionLegalHoldsOptionsArg = {} satisfies GetFileVersionLegalHoldsOptionsArg): Promise<any> {
        const response: FetchResponse = await fetch("".concat("https://api.box.com/2.0/file_version_legal_holds") as string, { method: "GET", params: { ["policy_id"]: policyId, ["marker"]: options.marker, ["limit"]: options.limit }, auth: this.auth, networkSession: this.networkSession } satisfies FetchOptions) as FetchResponse;
