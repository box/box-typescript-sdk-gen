...
export class TermsOfServicesManager {
    ...
    async getTermOfServices(options: GetTermOfServicesOptionsArg = {} satisfies GetTermOfServicesOptionsArg): Promise<any> {
        const response: FetchResponse = await fetch("".concat("https://api.box.com/2.0/terms_of_services") as string, { method: "GET", params: { ["tos_type"]: options.tosType }, auth: this.auth, networkSession: this.networkSession } satisfies FetchOptions) as FetchResponse;
