...
export class SearchManager {
    ...
    async postMetadataQueriesExecuteRead(requestBody: MetadataQuery): Promise<any> {
        const response: FetchResponse = await fetch("".concat("https://api.box.com/2.0/metadata_queries/execute_read") as string, { method: "POST", body: await serializeMetadataQuery(await serializeJson(requestBody)), auth: this.auth } satisfies FetchOptions) as FetchResponse;
        return await deserializeMetadataQueryResults(await deserializeJson(response.text));
    }
    async getMetadataQueryIndices(scope: GetMetadataQueryIndicesScopeArg, templateKey: string): Promise<any> {
        ...
        return await deserializeMetadataQueryIndices(await deserializeJson(response.text));
    }
