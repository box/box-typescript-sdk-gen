...
export class EventsManager {
    ...
    async getEvents(queryParams: GetEventsQueryParamsArg = {} satisfies GetEventsQueryParamsArg, headers: GetEventsHeadersArg = new GetEventsHeadersArg({})): Promise<Events> {
        ...
        const response: FetchResponse = await fetch("".concat("https://api.box.com/2.0/events") as string, { method: "GET", params: queryParamsMap, headers: headersMap, responseFormat: "json", auth: this.auth, networkSession: this.networkSession } satisfies FetchOptions) as FetchResponse;
        return deserializeEvents(deserializeJson(response.text));
    }
    async getEventsWithLongPolling(headers: GetEventsWithLongPollingHeadersArg = new GetEventsWithLongPollingHeadersArg({})): Promise<RealtimeServers> {
        const headersMap: {
            readonly [key: string]: string;
        } = prepareParams({ ...{}, ...headers.extraHeaders });
        const response: FetchResponse = await fetch("".concat("https://api.box.com/2.0/events") as string, { method: "OPTIONS", headers: headersMap, responseFormat: "json", auth: this.auth, networkSession: this.networkSession } satisfies FetchOptions) as FetchResponse;
