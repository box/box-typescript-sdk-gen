...
export class TermsOfServiceUserStatusesManager {
    ...
    async getTermOfServiceUserStatuses(tosId: string, options: GetTermOfServiceUserStatusesOptionsArg = {} satisfies GetTermOfServiceUserStatusesOptionsArg): Promise<any> {
        const response: FetchResponse = await fetch("".concat("https://api.box.com/2.0/terms_of_service_user_statuses") as string, { method: "GET", params: { ["tos_id"]: tosId, ["user_id"]: options.userId }, auth: this.auth, networkSession: this.networkSession } satisfies FetchOptions) as FetchResponse;
