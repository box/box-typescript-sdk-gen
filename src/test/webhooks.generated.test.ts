...
test("testWebhooksCRUD", async function testWebhooksCRUD(): Promise<any> {
    ...
    const webhook: any = await client.webhooks.createWebhook({ target: { id: folder.id, type: "folder" as CreateWebhookRequestBodyArgTargetFieldTypeField } satisfies CreateWebhookRequestBodyArgTargetField, address: "https://example.com/new-webhook", triggers: ["FILE.UPLOADED"] } satisfies CreateWebhookRequestBodyArg);
    ...
    const updatedWebhook: any = await client.webhooks.updateWebhookById(webhook.id, { address: "https://example.com/updated-webhook" } satisfies UpdateWebhookByIdRequestBodyArg);
    ...
    expect(async () => { await client.webhooks.deleteWebhookById(webhook.id); }).rejects.toThrow();
