...
test("test_create_update_get_delete_user", async function test_create_update_get_delete_user(): Promise<any> {
    ...
    const user: any = await client.users.createUser({ name: userName, login: userLogin, isPlatformAccessOnly: true } satisfies CreateUserRequestBodyArg);
    ...
    const updatedUser: any = await client.users.updateUserById(user.id, { name: updatedUserName } satisfies UpdateUserByIdRequestBodyArg);
