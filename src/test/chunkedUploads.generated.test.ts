...
export async function upload(client: Client): Promise<any> {
    ...
    const uploadSession: any = await client.chunkedUploads.createFileUploadSession({ fileName: fileName, fileSize: fileSize, folderId: folderId } satisfies CreateFileUploadSessionRequestBodyArg);
    ...
    async function reducer(acc: PartAccumulator, chunk: ByteStream): Promise<any> {
        ...
        const chunkSize: any = chunkBuffer.length;
    ...
    const results: any = await reduceIterator(chunksIterator, reducer, { lastIndex: -1, parts: [] } satisfies PartAccumulator);
    const parts: any = results.parts;
    const processedSessionParts: any = await client.chunkedUploads.getFileUploadSessionParts(uploadSessionId);
    if (!(processedSessionParts.totalCount == totalParts)) {
        throw "Assertion failed";
    }
    const processedSession: any = await client.chunkedUploads.getFileUploadSessionById(uploadSessionId);
