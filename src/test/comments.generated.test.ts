...
test("comments", async function comments(): Promise<any> {
    ...
    const uploadedFiles: any = await client.uploads.uploadFile({ attributes: { name: fileName, parent: { id: parentId } satisfies UploadFileRequestBodyArgAttributesFieldParentField } satisfies UploadFileRequestBodyArgAttributesField, file: fileByteStream } satisfies UploadFileRequestBodyArg);
    ...
    const newComment: any = await client.comments.createComment({ message: message, item: { id: fileId, type: "file" as CreateCommentRequestBodyArgItemFieldTypeField } satisfies CreateCommentRequestBodyArgItemField } satisfies CreateCommentRequestBodyArg);
    ...
    const newReplyComment: any = await client.comments.createComment({ message: message, item: { id: newComment.id, type: "comment" as CreateCommentRequestBodyArgItemFieldTypeField } satisfies CreateCommentRequestBodyArgItemField } satisfies CreateCommentRequestBodyArg);
    ...
    expect(async () => { await client.comments.getCommentById(newComment.id); }).rejects.toThrow();
