...
test("fileMetadata", async function fileMetadata(): Promise<any> {
    const client: any = new Client({ auth: new JwtAuth({ config: JwtConfig.fromConfigJsonString(decodeBase64(getEnvVar("JWT_CONFIG_BASE_64"))) }) });
    const uploadedFiles: any = await client.uploads.uploadFile({ attributes: { name: getUuid(), parent: { id: "0" } satisfies UploadFileRequestBodyArgAttributesFieldParentField } satisfies UploadFileRequestBodyArgAttributesField, file: generateByteStream(256) } satisfies UploadFileRequestBodyArg);
    ...
    const createdMetadata: any = await client.fileMetadata.createFileMetadataById(fileId, scope, template, data);
    ...
    const receivedMetadata: any = await client.fileMetadata.getFileMetadataById(fileId, scope, template);
    if (!(receivedMetadata.extraData.abc == data.abc)) {
        throw "Assertion failed";
    }
    const newValue: any = "bar";
    const updatedMetadata: any = await client.fileMetadata.updateFileMetadataById(fileId, scope, template, [{ op: "replace" as UpdateFileMetadataByIdRequestBodyArgOpField, path: "/abc", value: newValue } satisfies UpdateFileMetadataByIdRequestBodyArg]);
    const receivedUpdatedMetadata: any = await client.fileMetadata.getFileMetadataById(fileId, scope, template);
    if (!(receivedUpdatedMetadata.extraData.abc == newValue)) {
        throw "Assertion failed";
    }
    await client.fileMetadata.deleteFileMetadataById(fileId, scope, template)
    expect(async () => { await client.fileMetadata.getFileMetadataById(fileId, scope, template); }).rejects.toThrow();
