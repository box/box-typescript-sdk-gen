...
test('testEmailAliases', async function testEmailAliases(): Promise<any> {
  ...
  const newUser: any = await client.users.createUser({
    name: newUserName,
    login: newUserLogin,
  } satisfies CreateUserRequestBodyArg);
  const aliases: any = await client.emailAliases.getUserEmailAliases(
    newUser.id
  );
  ...
  const newAlias: any = await client.emailAliases.createUserEmailAlias(
    newUser.id,
    { email: newAliasEmail } satisfies CreateUserEmailAliasRequestBodyArg
  );
  const updatedAliases: any = await client.emailAliases.getUserEmailAliases(
    newUser.id
  );
  ...
  const finalAliases: any = await client.emailAliases.getUserEmailAliases(
    newUser.id
  );
