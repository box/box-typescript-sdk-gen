...
test("testMetadataTemplates", async function testMetadataTemplates(): Promise<any> {
    ...
    const template: any = await client.metadataTemplates.createMetadataTemplateSchema({ scope: "enterprise", displayName: templateKey, templateKey: templateKey, fields: [{ type: "string", key: "testName", displayName: "testName" }] } satisfies CreateMetadataTemplateSchemaRequestBodyArg);
    ...
    if (!await client.metadataTemplates.getMetadataTemplateSchema("enterprise" as GetMetadataTemplateSchemaScopeArg, template.templateKey)) {
        throw "Assertion failed";
    }
    const enterpriseMetadataTemplates: any = await client.metadataTemplates.getMetadataTemplateEnterprise();
    if (!(enterpriseMetadataTemplates.entries.length > 0)) {
        throw "Assertion failed";
    }
    const globalMetadataTemplates: any = await client.metadataTemplates.getMetadataTemplateGlobal();
    ...
    expect(async () => { await client.metadataTemplates.deleteMetadataTemplateSchema("enterprise" as DeleteMetadataTemplateSchemaScopeArg, template.templateKey); }).rejects.toThrow();
});
export {};
